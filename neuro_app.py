...
class PaintWidget(QWidget):
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        """
        Обрабатывает отпускание кнопки мыши. В режиме 'points' не завершает контур
        (это делается правым кликом в mousePressEvent). В режиме 'freehand' завершает контур.
        """
        if self.drawing_enabled and self.drawing_mode == 'freehand' and self.is_drawing:
            if event.button() == Qt.MouseButton.LeftButton:
